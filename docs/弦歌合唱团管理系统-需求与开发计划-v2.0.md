...
| 场景 | 行为 |
|------|------|
| 登录（团内用户） | `choir_slug` + **手机号** + `password`（+ 图形验证码） |
| 登录（系统超管） | **不传** `choir_slug`；**手机号** + `password`；JWT 无 `choir_id` |
...
> `username` 即手机号；注册成功后 `role_id` 为本团 `member`，`role_code=member`。
