...
| 项目名称 | 弦歌合唱团管理系统 |
| 文档版本 | **v2.1.9** |
| 编写日期 | 2026-05-19（v2.1.9 资料/作品/项目管理体验与手机端） |
| 状态 | 阶段一已完成；**阶段二资料/作品/项目管理已上线**（见 §8.5、§8.6、§9.3） |
...
平台面向 **一个或多个合唱团（团队）**：每团独立管理排练、献唱、作业、成员、资料、录音、**项目管理**（编年事项与归档）；**多个团队可同时使用同一套部署**，数据互不可见。每个团队注册成员 **不超过约 100 人**；**团队显示名称由该团管理员修改**。
...
**全量键（阶段一）**：`choir.rename`, `choir.suspend`, `members.read`, `members.write`, `members.assign_role`, `invites.create`, `invites.revoke`, `roles.manage`, `system.monitor`

**阶段二已纳入目录**（`GET /api/choir/permissions`）：`documents.read/write/*`、`works.read/write/*`、`recordings.read/write/*`、`projects.read/write/*`（角色管理页可勾选，如 **编辑作品信息** = `works.write`）
...
| 资料 | 上传、分类、预览/播放 | 二 |
| 录音 | 作品、多声部、播放 | 二 |
| 项目管理 | 编年纪类型、流水账、进度、待办；完成后摘要与项目资料（影视/图/文） | 二 |
...
- `/api/projects`（事项 CRUD、流水、待办、完成摘要、**项目资料** `assets`）
...
| `极简中式-项目管理.html` | **编年事项**：进行中记流水/待办/进度；完成后摘要与资料 | 管理壳 |
...
prototypes/
  ...
  js/projects-page.js     # 项目管理
  js/ui-dialog.js         # 全站统一 alert/confirm/prompt/toast
```
...
| 进入作品 | **点击作品名称** → `极简中式-乐谱详情.html?id={primary_score_id}`；无乐谱时 `?work_id=`；**列表无「操作」列** |
| 新建作品 | 搜索栏 **新建** → 弹窗名称/作曲者 → `POST /api/works` |
| 批量导入 PDF | **导入** 多选 PDF → 确认弹窗（可改作品名）→ 每文件创建作品并上传乐谱 |
| 改名 / 删除 | 在 **乐谱详情 · 作品基本信息** 中 **编辑**（名称/作曲者）或 **移入回收站**；不再在列表行内操作 |
| 所属团 | 标题旁 **所属团** 下拉 **仅当** `GET /api/auth/choir-contexts` 返回 `multiple: true`（一人多团或系统超管多团）时显示；单团用户自动用当前团 |
...
|------|------|
| 标题 | 显示 **作品名称**（面包屑/页签） |
| **作品基本信息** | 名称、作曲者；右侧 **编辑**（`works.write` 或 `documents.write`）→ `PATCH /api/works/:id`；**移入回收站**（团内管理角色，同原列表删除） |
| 乐谱介绍 | 富文本式多行说明，`PATCH` `documents.description` |
| **乐谱信息** | 编辑：所属作品（下拉）、分类、风格、合集、调性；上传人只读（原「基本信息」区） |
...
#### 8.5.6 资料管理 · 手机端布局（≤1200px）

| 项 | 说明 |
|----|------|
| 左侧菜单 | **不展示** 管理壳侧栏（`aside.sidebar` 隐藏）；导航依赖底部 Tab |
| 统计磁贴 | 顶部四格统计（资料总数等）**不展示**，直接 Tab + 列表 |
| 主内容 | `.content` 全宽；Tab 可横向滑动；表格可横向滚动 |
| 实现 | `极简中式-资料管理.html` 媒体查询 + `documents-page.js` `applyMobileSidebarLayout` |
...
#### 8.5.8 相关 API 摘要（作品/资料）

除 §7.4 路径外，已实现：

| 方法 | 路径 | 说明 |
|------|------|------|
| GET | `/api/auth/choir-contexts` | 当前账号可操作的团列表；`multiple: true` 时前端显示「所属团」 |
| GET | `/api/works` | 列表（含 `primary_score_id`、`score_count`；多团/超管带 `choir_id`） |
| POST | `/api/works` | 创建作品 |
...
> 分支 `cursor/requirements-v2.1-multitenant-plan`，2026-06 部署；权限键 `projects.read` / `projects.write`（总务默认含 `projects.*`）。

#### 8.6.1 列表与筛选

| 能力 | 说明 |
|------|------|
| 入口 | 管理壳侧栏 **项目管理** → `极简中式-项目管理.html` |
| 归档维度 | **年份** + **类型**（演出、排练、采购、行政、其他，可扩展） |
| 筛选 | 年份、类型、状态（进行中/已完成/已取消）、名称搜索 |
| 默认选中 | 列表加载后 **自动选中第一条** 事项并展示右侧详情（按 `updated_at` 降序） |
| 所属团 | 与资料管理相同：**仅多团身份** 显示顶栏所属团下拉（`choir-contexts`）；单团自动解析 |
...
| 待办 | 勾选完成、增删 |
| 流水账 | 收入/支出记账，列表展示结余 |
| 完成 | **完成项目并生成摘要**（服务端 `build_project_summary` 汇总进度、待办、流水） |

#### 8.6.3 已完成事项

| 区域 | 能力 |
|------|------|
| 完成摘要 | **置顶**显示；持 `projects.write` 可编辑并保存 |
| 隐藏 | 不再展示 **进度、待办、流水账**（API 亦禁止再改） |
| 项目资料 | Tab：**影视** / **图** / **文**；导入文件至 CDE；影视可 **添加视频链接** |
| 文件类型 | 影视：mp4/mov/webm 等；图：jpg/png/webp 等；文：pdf/doc/docx/txt/xls/ppt 等 |

#### 8.6.4 相关 API 与表

| 方法 | 路径 | 说明 |
|------|------|------|
| GET | `/api/projects/meta` | 年份、类型元数据 |
| GET/POST | `/api/projects` | 列表/新建 |
| GET/PATCH/DELETE | `/api/projects/:id` | 详情/更新/删除 |
| POST | `/api/projects/:id/complete` | 标记完成并生成摘要 |
| POST/DELETE | `/api/projects/:id/transactions` | 流水（进行中） |
| POST/PATCH/DELETE | `/api/projects/:id/todos` | 待办（进行中） |
| POST/DELETE | `/api/projects/:id/assets` | 项目资料上传/删除（**仅已完成**） |
| GET | `/api/projects/:id/assets/:aid/stream` | 资料文件流（mock/PDS） |

表：`projects`、`project_transactions`、`project_todos`、`project_assets`（`media_kind`: `video` / `image` / `text`）。

迁移：`scripts/migrations/20260601_projects.sql`、`scripts/migrations/20260602_project_assets.sql`。
...
| 2.7 | 项目管理：流水/待办/进度、完成摘要、项目资料（影视/图/文） | §8.6、`projects-page.js` | ✅ 已上线 |

**验收（当前）**：
- 作品列表可新建/导入；改名/删除在乐谱详情「作品基本信息」；列表无操作列。
- 乐谱详情可编辑作品信息与乐谱元数据、上传伴奏与视频、PDF 内联预览（PDS）。
- 资料管理：多团才显示「所属团」；手机端无侧栏、无统计磁贴。
- 登录页支持已保存账号与可选记住密码。
- 窄屏管理页有底部 Tab 导航。
- 项目管理：默认选中首条；进行中可记流水与待办；完成后摘要置顶，可导入影视/图/文资料。
- 角色管理可勾选 `works.write`（编辑作品信息）等阶段二权限键。
...
| **v2.1.8** | 2026-05-19 | 新增 §8.6 项目管理：编年事项、流水/待办/进度、完成摘要置顶、已完成项目资料（影视/图/文）；任务 2.7 |
| **v2.1.9** | 2026-05-19 | §8.5：作品列表去操作列、详情页作品基本信息编辑/回收站、`works.*` 权限、多团所属团、`choir-contexts`、手机端无侧栏与统计磁贴；§8.6：默认选中首条项目 |
| v2.1 | 计划 | 阶段一完成 |
| v2.2 | 计划 | 阶段二完成 |
| v2.3 | 计划 | 阶段三完成 |
| v2.4 | 计划 | 正式上线 |
